...
def generate_text_completion_requests(
    ...
    for pair in data:
        ...
        if input_len + output_len < max_seq_len and output_len > 0:
    ...
    for timestamp, pair in zip(timestamps, data):
        ...
        if cum_sum < n1 * num_requests:
            sampling_params = SamplingParams(
                n=1, max_tokens=output_len, **random_sampling_params_dict)
        elif cum_sum < (n1 + n2) * num_requests:
            sampling_params = SamplingParams(
                n=2, max_tokens=output_len, **random_sampling_params_dict)
        elif cum_sum < (n1 + n2 + n3) * num_requests:
            sampling_params = SamplingParams(
                n=3, max_tokens=output_len, **random_sampling_params_dict)
        elif cum_sum < (n1 + n2 + n3 + n4) * num_requests:
            sampling_params = SamplingParams(
                n=4, max_tokens=output_len, **random_sampling_params_dict)
        elif cum_sum < (n1 + n2 + n3 + n4 + n6) * num_requests:
            sampling_params = SamplingParams(
                n=6, max_tokens=output_len, **random_sampling_params_dict)
        elif cum_sum < (n1 + n2 + n3 + n4 + n6 + n2_beam) * num_requests:
            sampling_params = SamplingParams(
                n=2, max_tokens=output_len, **beam_search_params_dict)
        elif cum_sum < (n1 + n2 + n3 + n4 + n6 + n2_beam + n4_beam) * num_requests:
            sampling_params = SamplingParams(
                n=4, max_tokens=output_len, **beam_search_params_dict)
        elif cum_sum < (n1 + n2 + n3 + n4 + n6 + n2_beam + n4_beam + n6_beam) * num_requests:
            sampling_params = SamplingParams(
                n=6, max_tokens=output_len, **beam_search_params_dict)
        elif cum_sum < (n1 + n2 + n3 + n4 + n6 + n2_beam + n4_beam + n6_beam + n8_beam) * num_requests:
            sampling_params = SamplingParams(
                n=8, max_tokens=output_len, **beam_search_params_dict)
        else:
...
def generate_throughput_requests(
    dataset: str,
    num_requests: float,
    seed: int,
    n1: float = 0.0,
    ...
    n8_beam: float = 0.0,
    max_seq_len: int = 2048,
) -> List[Tuple[List[int], SamplingParams]]:
    random.seed(seed)
    np.random.seed(seed)
    ...
    assert n1 + n2 + n3 + n4 + n6 + n2_beam + n4_beam + n6_beam + n8_beam == 1.0
    cum_sum = 0
    for input_tokens, output_len in data:
        if cum_sum < n1 * num_requests:
            sampling_params = SamplingParams(
                n=1, max_tokens=output_len, **random_sampling_params_dict)
        elif cum_sum < (n1 + n2) * num_requests:
            ...
            raise ValueError('Invalid request ratio.')
        cum_sum += 1
        requests.append((input_tokens, sampling_params))
    return requests